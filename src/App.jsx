...
function App() {
  ...
  function booking(id) {
    ...
    setInfo(id);
    document.getElementById('one').classList.remove('d-sm-flex');
    document.getElementById('one').classList.add('d-none');
    document.getElementById('two').classList.remove('d-none');
    document.getElementById('two').classList.add('d-sm-flex');
    document.getElementsByTagName('h1')[0].innerHTML = id.name;
    document.getElementsByTagName('input')[0].value = id.name;
  }
  function setLimit() {
    const minDate = document.getElementsByTagName('input')[1].value;
    document.getElementsByTagName('input')[2].min = minDate;
  }
  function bDet(event) {
    event.preventDefault();
    const bookingData = {
      origin: document.getElementsByTagName('select')[0].value,
      destination: document.getElementsByTagName('input')[0].value,
      from: document.getElementsByTagName('input')[1].value,
      to: document.getElementsByTagName('input')[2].value
    };
  ...
  return (
    <div className='main-bg-img pb-5' style={{backgroundImage: `linear-gradient(to right, rgba(0, 0, 0, 0.6), rgba(0, 0, 0, 0.6)), url('${bgImg}')`}}> 
      ...
      <div className='container pb-5'>
        <div id='one' className='d-sm-flex justify-content-between'>

          {spots.map(x => <div style={{ backgroundImage: `url(${x.bgURL})`}} className='text-white mb-5 mx-auto place-boxes text-center rounded-5' key={x.id}> <h3 className='ttm'>{x.name}</h3> <br /> <button onClick={() => { booking(x) }} type='button' className='btn btn-primary px-4 btn-color border-0'>Booking <i className="bi bi-arrow-right"></i></button></div>)}

        </div>
        <div id='two' className='d-none text-white justify-content-between'>
          <div className='w-50 w-fix mb-5'><h1 className='fs-h1'></h1>
          ...
          <form onSubmit={bDet} className='form-styling p-4 rounded'>
            ...
            <p>Origin :</p>
            <select className='bg-light w-100 border-0 fw-bold'>
              <option>Dhaka</option>
